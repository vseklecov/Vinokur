...
void DistillationProcess::service()
{
  ...
  if (boiling_kub==0.0 && _state==STATE_HEAT && _sp->getThempKub()>_temp_cool_on) {
    if (temp_boiling==0.0) {
      temp_boiling = _sp->getThempKub();
      time_boiling = millis();
    }
    if ((millis()-time_boiling)>20000) { // 20 секунд
...
void FractioningProcess::service()
{
  ...
  if (boiling_kub==0.0 && _state==STATE_HEAT) {
    if (temp_boiling==0.0) {
      temp_boiling = _sp->getThempKub();
      time_boiling = millis();
    }
...
void FractioningProcess::readDataEEPROM()
{
  ...
  if (EEPROM[0]==1) {  // EEPROM с параметрами для перегонки браги, записать параметры для дробной перегонки
    _temp_cool_on        = 75.0;
    _temp_def_cool_on    = 50.0;
    _temp_distil_off     = 98.0;  // 8%
    _temp_def_distil_off = 99.1;  // 10%
